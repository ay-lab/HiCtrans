...
buildMatrixPath <- "/home/abhijit/overflow/proj_overflow/HiCProcessingPipeLine/Cancer_Cell_Lines/HiCtrans_Calls/HiCtrans/build_matrix"
...
di_and_hmm <- function(coverage,chromA,chromA.start,chromA.end,chromB,chromB.start,chromB.end,prefix,id){
  ...
  bed$up[bed$up == 0] <- rbinom(length(bed$up[bed$up == 0]), 3, 0.75)
  bed$dw[bed$dw == 0] <- rbinom(length(bed$dw[bed$dw == 0]), 3, 0.75)
  bed$trans[bed$trans == 0] <- rbinom(length(bed$trans[bed$trans == 0]), 3, 0.75)
  ...
  bed_chromA <- as.data.frame(cbind(bed_chromA,bed_chromA_cis_di,bed_chromA_trans_di))
  ...
  bed_chromB <- as.data.frame(cbind(bed_chromB,bed_chromB_cis_di,bed_chromB_trans_di))
...
EstimateMaxRegion <- function(box, mat, locq) {
  ...
  #b <- as.data.frame(do.call(rbind,b)) 
  b <- as.data.frame(data.table::rbindlist(b))
  return(b)
}
...
BoxEnrichment <- function(m, x, y, core, boxzscore, cutoff=0.05) {
...
GetSegments <- function(mat, bed, chromA, chromB, prefix, covq=0.75, locq, mincount, minzscore, resolution, glbq, boxzscore) {
  ...
  if (nrow(r.df) > 0 & nrow(c.df) > 0) {
    ...
    box <- BoxEnrichment(mat,r.df,c.df,core,boxzscore)
    if (nrow(box) > 0) {
    ...
    if (covq > 0.5) {
      ...
      GetSegments(mat, bed, chromA, chromB, prefix, covq=(covq-0.25), locq, mincount, minzscore, resolution, glbq, boxzscore)
    } else {
...
option_list = list(
 ...
 \t\t.....\n"),
  ...
  make_option(c("--precheck"), default=1, type="numeric", help="Precheck option will help to restrict HiCtrans search only to chromosome combinations with significant max interaction compared to mean non-zero count value. [Default 1. Lower value will increase stringency]")
)
...
pval <- ppois(max(mat$mat), mean(mat$mat[mat$mat > 0]), lower.tail=F)
if (pval >= opt$precheck) {
  cat("Max count of highest resolution Hi-C matrix is non-significant. HiCtrans will not search this chromosome combination.\n")
  quit(save="no")
} else {
  cat ("Max count of highest resolution Hi-C matrix is significant. HiCtrans will go-ahead to search translocation.\n")
}
...
for (i in 1:length(all.resolution)) {
  ...
  boxzscore<- as.integer(opt$boxzscore)
  prefix_resolution <- paste0(prefix,"_",as.integer(all.resolution[i]))
  print (paste0("Running for ",all.resolution[i]," resolution"))
  translocation.boxes[[i]] <- GetSegments(mat$mat[[i]], mat$bed[[i]], chromA, chromB, prefix_resolution, covq, locq, mincount, minzscore, as.integer(all.resolution[i]), glbq, boxzscore)
      ...
      while (k <= nrow(param)){
        ...
        boundary <- di_and_hmm(
         ...
         if (is.null(chromA_BP_start[k][[1]])) {
           ...
           chromA_BP_end[k]   <- tail(df$bed[df$bed$chrom==as.character(opt$chrA) & df$bed$end <= floor(bp_optimized$par[1]),]$end,1)
         }
         if (is.null(chromB_BP_start[k][[1]])) {
